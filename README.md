...
- 자동 최적 회전(세움+눕힘 6방향) / 세움 유지 / 옆으로 눕힘 / 앞으로 눕힘 / 방향 고정
...
- **4가지 최적화 전략 자동 비교**: 대수 최소 / 40ft 우선 / 20ft 우선 / 활용률 우선
- 자동 모드에서 동률이면 더 작은 총 컨테이너 용량과 높은 활용률을 우선
- 화물 표에서 **수량과 눕힘/회전 조건 즉시 수정**
...
첨부 CAD 사례를 재현하기 위한 예시는 `examples/cad-layout-3-container-example.csv`에 포함했습니다. 기존 입력 파일은 4단 수세가 `35`개, 1단 수세가 `upright`로 되어 있어 컨테이너가 과도하게 늘어날 수 있습니다. 새 버전은 계산 전에 이와 같은 체적 편중과 세움 고정 조건을 화면에서 경고합니다. 4단 수세 수량을 `3`, 1단 수세 회전을 `free`로 두고 기본 20STD/40STD/40HC 후보를 모두 허용하면 현재 휴리스틱의 **자동 비교 추천 결과는 40STD 2대 + 20STD 1대, 총 3대**입니다. 실제 포장 외형·중량·현장 여유를 넣으면 결과가 달라질 수 있습니다.
