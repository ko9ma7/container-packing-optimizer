...
function renderAppShell() {
  $('#app').innerHTML = `
    ...
    <footer class="site-footer"><span data-i18n="footer"></span><span class="build-version" title="Expanded world city index and city live insights">V8.0</span><button type="button" id="footerInfo" class="text-button" data-i18n="dataSources"></button></footer>
...
function renderCityInsights() {
  ...
  if (state.destination?.kind === 'country') {
    ...
    root.innerHTML = `<div class="country-city-directory"><h4>${escapeHtml(t('countryCities'))}</h4><p>${escapeHtml(t('cityDirectoryNote'))}</p><div class="country-city-grid">${cities.map((city, index) => `<button type="button" data-country-city-index="${index}"><span><strong>${escapeHtml(city.name)}</strong>${city.admin1 ? `<small>${escapeHtml(city.admin1)}</small>` : ''}</span>${city.population ? `<em>👥 ${escapeHtml(formatCompact(city.population))}</em>` : '<em>→</em>'}</button>`).join('')}</div></div><p class="city-source-note">GeoNames cities15000 · CC BY 4.0</p>`;
  ...
  let overview = '';
  if (wiki?.extract) {
    overview = `<div class="city-overview-card">${wiki.image ? `<img src="${escapeHtml(wiki.image)}" alt="${escapeHtml(wiki.title || state.destination.name)}" loading="lazy" referrerpolicy="no-referrer">` : ''}<div><span class="city-subheading">${escapeHtml(t('cityOverview'))}</span><p>${escapeHtml(wiki.extract)}</p>${wiki.url ? `<a href="${escapeHtml(wiki.url)}" target="_blank" rel="noreferrer">${escapeHtml(t('wikipediaSource'))} ↗</a>` : ''}</div></div>`;
  }
  ...
  root.innerHTML = `${overview}${referenceBlock}<div class="city-live-group"><h4>${escapeHtml(t('cityDetails'))}</h4><div class="city-metrics-grid">${cityFacts}</div></div>${weatherBlock}${airBlock}${nearbyBlock}${otherCitiesBlock}${degraded ? `<p class="city-warning">${escapeHtml(t('cityInfoUnavailable'))}</p>` : ''}<p class="city-source-note">${escapeHtml(t('cityDataSources'))}</p>`;
...
async function hydrateCountryCities() {
  ...
  renderCityInsights();
  const rows = await getCountryCitySuggestions(code2, { limit: 30 }).catch(() => []);
...
async function selectResult(result, { skipUrl = false } = {}) {
  ...
  resolveRoadRoute();
  const recentItem = { key: state.destination.key, name: state.destination.name, countryCode: code2, kind: state.destination.kind, lat: state.destination.lat, lon: state.destination.lon, timezone: state.destination.timezone, rawName: state.destination.rawName, admin1: state.destination.admin1, population: state.destination.population, geonameId: state.destination.geonameId };
