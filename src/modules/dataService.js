...
export async function searchLocal(query, locale) {
  ...
  if (!q) return [];
  const [countries, places, cities] = await Promise.all([loadCountryIndex(), loadPlaces(), loadCities()]);
  ...
  for (const row of cities) {
    ...
    const score = Math.min(scoreField(name, q, locale), scoreField(admin1, q, locale));
...
export async function getCountryCitySuggestions(countryCode, { admin1 = '', excludeName = '', limit = 12 } = {}) {
  ...
  for (const row of rows) {
    ...
    seen.add(key);
    candidates.push({
      kind: 'city-index', type: 'city', id: `gn-${geonameId}`, geonameId, name,
